...
class TensorboardLogger(SupervisorEnv):
    ...
    def step(self, action_n):
        ...
        isDone_n = []
        info_n = []
        new_obs_n, reward_n, isDone_n, info_n = self.controller.step(action_n)
        ...
        if (self.v_reward > 1):
            self.file_writer.add_scalar("Rewards/Per Global Step", reward_n,
                                        self.step_global)
        ...
        for i in range(len(reward_n)):
            if reward_n[i]:
                self.score += reward_n[i]

        self.step_cntr += 1
        self.step_global += 1

        return new_obs_n, reward_n, isDone_n, info_n
