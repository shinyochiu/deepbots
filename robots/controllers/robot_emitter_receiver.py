...
class RobotEmitterReceiver(ABC):
    ...
    def __init__(self,
        ...
        self.get_comms_id(self.robot.getName())

        self.emitter, self.receiver = self.initialize_comms(
            self.emitter_name, self.receiver_name)

    def get_comms_id(self, name):
        if "LEADER" in name:
            self.emitter_name +="_0"
            self.receiver_name +="_0"
        else:
            self.emitter_name += "_" + name[name.find("FOLLOWER_") + 9:]
            self.receiver_name += "_" + name[name.find("FOLLOWER_") + 9:]
